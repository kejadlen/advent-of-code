class Room
  attr_reader :encrypted_name, :sector_id

  def initialize(encrypted_name, sector_id)
    @encrypted_name, @sector_id = encrypted_name, sector_id
  end
...
if __FILE__ == $0
  DATA.read.lines.map {|line|
    /(?<name>.+)-(?<sector_id>\d+)\[(?<checksum>[a-z]+)\]/ =~ line
...
class TestDay04 < Minitest::Test
  def test_day_04
    assert_equal 'abxyz', Room.new('aaaaa-bbb-z-y-x', 123).checksum
